...
# Doc 3 roadmap (channel choice by tier, 2026-09-09) — how many channels a tier can connect, not
# which specific ones: a business freely picks any combination of WhatsApp/Instagram/Messenger up
# to this count (superseded the earlier fixed WhatsApp->+Messenger->+Instagram ladder, which
# forced every tier through WhatsApp first — this instead lets e.g. a Starter business pick
# Instagram-only, never touching WhatsApp at all). The source of truth api/onboarding.py's
# whatsapp_signup_callback/page_signup_callback, templates/dashboard.html's toolbar, and the
# signup/billing tier-comparison cards all read from, so they can never drift from each other.
# Enforced against len(Business.channels_connected) — deliberately not retroactive, same
# convention as CATALOG_LIMIT above: a business already over a newly-lower limit keeps what's
# already connected, only new connects are blocked.
CHANNEL_LIMIT: dict[PlanTier, int] = {
    PlanTier.starter: 1,
    PlanTier.growth: 2,
    PlanTier.pro: 3,
}
...
def tier_comparison_rows() -> list[dict]:
    ...
    feature gate that isn't its own dict (billing/tiers.py's CATALOG_LIMIT/CHANNEL_LIMIT are;
